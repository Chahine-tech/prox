...
use crate::config::models::{ServerConfig, RouteConfig, RateLimitConfig, TlsConfig, AcmeConfig};
...
pub enum ValidationError {
    ...
    InvalidUrl { field: String, url: String, reason: String },
...
impl ConfigValidator {
    ...
    fn validate_listen_address(address: &str) -> ValidationResult<()> {
        if address.parse::<SocketAddr>().is_err() {
            return Err(ValidationError::InvalidListenAddress {
                ...
                reason: "Must be in format 'IP:PORT' (e.g., '127.0.0.1:3000' or '0.0.0.0:8080')".to_string(),
            });
        }
        Ok(())
    }
            ...
            RouteConfig::LoadBalance { targets, .. } => {
                if targets.is_empty() {
                    ...
                    for (i, target) in targets.iter().enumerate() {
                        if let Err(e) = Self::validate_url(target, &format!("route '{path}' load balance target {i}")) {
                            errors.push(e);
                        }
                    }
                }
    ...
    fn validate_rate_limit(route_path: &str, config: &RateLimitConfig) -> ValidationResult<()> {
        ...
        if let Err(e) = Self::parse_period(&config.period) {
            return Err(ValidationError::InvalidRateLimit {
                ...
                message: format!("Invalid period format '{}': {}. Use formats like '1s', '1m', '1h'", config.period, e),
            });
        }
        ...
        if config.status_code < 400 || config.status_code > 599 {
            return Err(ValidationError::InvalidRateLimit {
                route: route_path.to_string(),
                message: format!("Status code {} is not valid for rate limiting. Use 4xx or 5xx codes", config.status_code),
            });
        }
        ...
        if let crate::config::models::RateLimitBy::Header = config.by {
            if let Some(header_name) = &config.header_name {
                if header_name.is_empty() {
                    return Err(ValidationError::InvalidRateLimit {
                        route: route_path.to_string(),
                        message: "header_name cannot be empty when rate limiting by header".to_string(),
                    });
                }
                
                // Validate header name format
                if let Err(_) = header_name.parse::<hyper::header::HeaderName>() {
                    return Err(ValidationError::InvalidRateLimit {
                        route: route_path.to_string(),
                        message: format!("Invalid header name '{header_name}'. Header names must be valid HTTP header names"),
                    });
                }
            } else {
                return Err(ValidationError::InvalidRateLimit {
    ...
    fn validate_tls_config(config: &TlsConfig) -> ValidationResult<()> {
        match (&config.cert_path, &config.key_path, &config.acme) {
            ...
            (None, None, Some(acme_config)) => {
                // ACME configuration
                if let Err(e) = Self::validate_acme_config(acme_config) {
                    return Err(e);
                }
            },
    ...
    fn validate_acme_config(config: &AcmeConfig) -> ValidationResult<()> {
        ...
        if let Some(days) = config.renewal_days_before_expiry {
            if days == 0 || days > 89 {
                return Err(ValidationError::InvalidAcme {
                    message: format!("renewal_days_before_expiry must be between 1 and 89, got: {days}"),
                });
            }
        }
    ...
    fn check_route_conflicts(routes: &HashMap<String, RouteConfig>) -> Result<(), Vec<ValidationError>> {
        let mut errors = Vec::new();
    ...
    fn routes_conflict(path1: &str, path2: &str) -> bool {
        ...
        let longer = if path1_norm.len() > path2_norm.len() { path1_norm } else { path2_norm };
        let shorter = if path1_norm.len() <= path2_norm.len() { path1_norm } else { path2_norm };
        
        longer.starts_with(shorter) && (
            longer.len() == shorter.len() ||
            longer.chars().nth(shorter.len()) == Some('/')
        )
    }
    ...
    fn parse_period(period: &str) -> Result<Duration, String> {
        ...
        let (number_part, unit_part) = if let Some(pos) = period.chars().position(|c| c.is_alphabetic()) {
            period.split_at(pos)
        } else {
        ...
        let duration = match unit_part.to_lowercase().as_str() {
            ...
            _ => return Err(format!("Invalid time unit: {unit_part}. Use 's', 'm', or 'h'")),
        };
...
mod tests {
    ...
    fn create_valid_config() -> ServerConfig {
        ...
        routes.insert("/test".to_string(), RouteConfig::Proxy {
    ...
    fn test_invalid_listen_address() {
        ...
        assert!(result.unwrap_err().to_string().contains("Invalid listen address"));
    }
    ...
    fn test_missing_routes() {
        ...
        let result = ConfigValidator::validate(&config);
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Missing required field: routes"));
    }
    
    #[test]
    fn test_invalid_proxy_url() {
        let mut config = create_valid_config();
        config.routes.insert("/test".to_string(), RouteConfig::Proxy {
