    ...
    pub async fn run(&self) -> Result<()> {
        ...
        loop {
            ...
            for backend_entry in self.proxy_service.backend_health().iter() {
                ...
                match self.http_client.health_check(&health_check_url, timeout.as_secs()).await {
                    Ok(is_healthy) => {
                        if is_healthy {
                            ...
                            tracing::info!("Health check for {} succeeded ({} consecutive successes)", 
                                         target, successes);
                            ...
                            self.handle_health_check_failure(&target, backend_health, health_config, 
