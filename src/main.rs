...
enum RouteConfig {
    ...
    Redirect { target: String, status_code: Option<u16> },
    #[serde(rename = "proxy")]
    Proxy { target: String },
    #[serde(rename = "load_balance")]
    LoadBalance { targets: Vec<String>, strategy: LoadBalanceStrategy },
}
...
async fn main() -> anyhow::Result<()> {
    ...
    let config_content = match fs::read_to_string(&args.config).await {
        Ok(content) => content,
        Err(err) => {
            tracing::error!("Failed to read config file: {}, error: {}", args.config, err);
    ...
    let addr: SocketAddr = match config.listen_addr.parse() {
        Ok(addr) => addr,
        Err(err) => {
            tracing::error!("Invalid listen address: {}, error: {}", config.listen_addr, err);
        ...
        Some(route_config) => {
            match route_config {
                RouteConfig::Static { root } => {
                    handle_static(root, matched_prefix, req).await
                },
...
async fn handle_redirect(target: &str, path: &str, prefix: &str, status_code: Option<u16>) -> Response {
    let rel_path = &path[prefix.len()..];
...
async fn handle_proxy(client: &HyperClient, target: &str, mut req: Request<Body>, prefix: &str) -> Response {
    let path = req.uri().path();
    let rel_path = &path[prefix.len()..];
